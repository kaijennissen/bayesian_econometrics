...
for isim in 1:nsim+burnin
    ...
    if sum(phic) < .99 && phic[2] - phic[1] < .99 && phic[2] > -.99
        global phi = phic;
        global Hphi = sparse_diag - phi[1]*sparse_subdiag_1 - phi[2]*sparse_subdiag_2;
        global HHphi = Hphi'*Hphi;
        global count_phi = count_phi + 1;
    end

    # # sample sigc2
    G = Gamma(nu_sigc2 + T/2, 1/(S_sigc2 + .5*(c'*HHphi*c)[1]));
    global sigc2 = 1 / rand(G);
    ...
    global sigtau2 = sigtau2_grid[rand(Uniform()).<cdf_sigtau2][1];
    ...
    global tau0 = tau0_hat + Ctau0.U \ randn(2);
