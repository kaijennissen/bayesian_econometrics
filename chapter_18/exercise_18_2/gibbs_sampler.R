...
for (ii in 1:(nsim + nburn)) {
  ...
  omega2 <- 1 / rgamma(n = 1, shape = ny_sigma2_c + TT / 2, scale = 1 / as.matrix(S_sigma2_c_temp))
  
  # draw from conditional for sigma^2_tau
  del_tau <- c(tau0[1], tau[1:TT])-c(tau0[2],tau0[1],tau[1:(TT-1)])
  f_tau <- function(x){-TT/2*log(x)-sum(diff(del_tau)[-TT]**2)/(2*x)}
  #sum((del_tau[2:TT]-del_tau[1:(TT-1)])**2)
  sigma2_tau_grid <- seq(from = runif(1)/1000,to = b_sigma2_tau-runif(1)/1000,length.out = 500)
  ...
  if (ii > nburn) {
    ...
    store_phi[nn, ] <- phi[, , drop = T]
    store_gamme[nn, ] <- gamma[, , drop = T]
