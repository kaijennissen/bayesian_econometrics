...
microbenchmark::microbenchmark(
    ...
    Diagonal(TT) + sparseMatrix(2:TT,  1:(TT - 1), x = rep(-2, TT - 1), dims = c(TT, TT)) +
        sparseMatrix(3:TT,  1:(TT - 2), x = rep(1, TT - 2), dims = c(TT, TT)),
    times = 1000,
    unit = "s"
)
...
microbenchmark::microbenchmark(crossprod(H_2), t(H_2) %*% H_2)
...
for (ii in 1:total_runs) {
    ...
    tau <- tau_hat + solve(L_tau, diag(ncol(L_tau))) %*% Z_tau
    ...
    if (ii > nburn) {
        ...
        store_tau[, nn] <- tau@x
        store_theta[, nn] <-
            c(phi@x, sigma2_c, sigma2_tau, gamma@x)
    }
}
print(Sys.time() - now)
